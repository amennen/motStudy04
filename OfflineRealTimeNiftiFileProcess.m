function [patterns, t] = OfflineRealTimeNiftiFileProcess(subjectNum,featureSelect,prev,scanNow,scanNum,SESSION,blockNum,runNum,roi_name) %,rtfeedback)
...
%load trained model
allfn = dir([locPatterns_dir '*AVGloctrainedModel_' num2str(runNum) '*']); %
...
allLast = dir([locPatterns_dir '*AVGlocpatternsdata_' '*']);
...
save([runHeader 'AVGmotpatternsdata_' num2str(SESSION) '_' datestr(now,30)],'patterns', 't');
