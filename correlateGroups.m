...
sepTRs = 17;
nblock = 3;
svec = [12:16 18 20 21 22 24 26]; %[3:5 7]; %subjects 3,4,5,7 are for initial RT, subjects 8-10 are after changes
RT = [12:15 18 21:22];
YC = [16 20 24 26];
YM = [8 14 13 15];
...
for s = 1:nsub
    ...
    for iblock = 1:nblock
        ...
        allschange = d.stim.changeSpeed;
        SCV = reshape(allschange,1,numel(allschange));
        if ismember(svec(s),YC) %now we get the speed changes from yoked person
            iYC = find(YC == svec(s));
            s2 = YM(iYC);
            b_dir2 = [fileparts(which('mot_realtime01.m')) '/BehavioralData/' num2str(s2) '/'];
            fileSpeed = findNewestFile(b_dir2,fullfile(b_dir2, ['mot_realtime01_' num2str(s2) '_' num2str(SESSION)  '*.mat']));
            d = load(fileSpeed);
            allSpeed = d.stim.motionSpeed; %matrix of TR's
            allschange = d.stim.changeSpeed;
            SCV = reshape(allschange,1,numel(allschange));
        ...
        scbytrial = reshape(SCV,nTRs,nstim);
        scbytrial = scbytrial';
        scinorder = scbytrial(indSort,:);
        scbystim(:,(iblock-1)*nTRs + 1: iblock*nTRs ) = scinorder;
...
%    end
    ...
    print(h, sprintf('%sALLNEWCORR.pdf', plotDir), '-dpdf')
