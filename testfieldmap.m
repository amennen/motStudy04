...
APname = 'SE_AP';
AP_re = [APname '_re'];
unix(sprintf('%sdicom2bxh %s001_00000%s_0* %s.bxh',bxhpath,imgDir,num2str(scanNum),APname));
...
PAname = 'SE_PA';
PA_re = [PAname '_re'];
unix(sprintf('%sdicom2bxh %s001_00000%s_0* %s.bxh',bxhpath,imgDir,num2str(scanNum),PAname));
%reorient bxh wrapper
...
imgDir = ['/Data1/subjects/' datestr(allDates{1},10) datestr(allDates{1},5) datestr(allDates{1},7) '.' subjectName '.' subjectName '/'];
subjNum = 5;
dataDir = ['/Data1/code/' projectName '/data/' num2str(subjNum) '/']; %this is where she sets the save directory!
process_dir = [dataDir 'reg' '/'];
scanNum = 11;
fileNum = 636;
bxhpath='/opt/BXH/1.11.1/bin/';
cd(process_dir)
testfn = 'testexfun';
testRE = 'testex_reorient';
addpath(genpath('../../../'))
[~,filename] = GetSpecificFMRIFile(imgDir,scanNum,fileNum)

% first convert to nifti
unix(sprintf('%sdicom2bxh %s%s %s.bxh',bxhpath,imgDir,filename,testfn));
%reorient bxh wrapper
unix(sprintf('%sbxhreorient --orientation=LAS %s.bxh %s.bxh',bxhpath,testfn,testRE));
%convert the reoriented bxh wrapper to a nifti file
unix(sprintf('%sbxh2analyze --overwrite --analyzetypes --niigz --niftihdr -s %s.bxh %s',bxhpath,testRE,testRE))
%% now read in nifti file
addpath(genpath('/Data1/code/multibandutils'));
[datafromnifti] = readnifti(sprintf('%s.nii.gz',testRE));
[datafromnifti] = readnifti(sprintf('%s.nii',testRE));
% rotate nifti 90
niftiRot = zeros(size(datafromnifti));
for i = 1:size(datafromnifti,3)
    niftiRot(:,:,i) = rot90(datafromnifti(:,:,i)); %rotates entire slice by 90 degrees
end
if (size(roi,1)>1)
    niftivec = niftiRot(roi);
end   
% now compare with dicom loading
brainmask = ones(64,64,36);
roi = logical(brainmask);
[datafromdicom,~] = ReadFile(sprintf('%s%s',imgDir,filename),64,roi);
...
scanNum = 2%5;%should be 5-change back!!
